...
task load_lineage_db: :environment do
  ...
  ` mkdir -p #{local_taxonomy_path};
    ...
    mysqlimport --delete --local --user=$DB_USERNAME --host=#{rds_host} --password=$DB_PASSWORD --ignore-lines=1 --columns=taxid,superkingdom_taxid,phylum_taxid,class_taxid,order_taxid,family_taxid,genus_taxid,species_taxid --fields-terminated-by=',' idseq_#{Rails.env} taxon_lineages;

    mv #{names_file} taxon_names;
    mysqlimport --delete --ignore-lines=1 --local --user=$DB_USERNAME --host=#{rds_host}  --columns=taxid,name --password=$DB_PASSWORD --fields-terminated-by=',' idseq_#{Rails.env} taxon_names;

    for taxl in species genus family order class phylum superkingdom ; do
      # print out sql command
      echo "update taxon_lineages inner join taxon_names on taxon_lineages.${taxl}_taxid = taxon_names.taxid set taxon_lineages.${taxl}_name = taxon_names.name;" | mysql --user=$DB_USERNAME --host=#{rds_host} --password=$DB_PASSWORD idseq_#{Rails.env};
    done
    cd /app;
