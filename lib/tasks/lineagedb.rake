...
task load_lineage_db: :environment do
  ...
  ` mkdir -p #{local_taxonomy_path};
    ...
    mv #{common_names_file} taxon_common_names;
    mysqlimport --delete --ignore-lines=1 --local --user=$DB_USERNAME --host=#{host}  --columns=taxid,name --password=$DB_PASSWORD --fields-terminated-by=',' idseq_#{Rails.env} taxon_common_names;
    ...
    for taxl in species genus family order class phylum superkingdom ; do
      ...
      echo "update taxon_lineages inner join taxon_common_names on taxon_lineages.${taxl}_taxid = taxon_common_names.taxid set taxon_lineages.${taxl}_common_name = taxon_common_names.name;" | mysql --user=$DB_USERNAME --host=#{host} --password=$DB_PASSWORD idseq_#{Rails.env};
    done
    cd /app;
