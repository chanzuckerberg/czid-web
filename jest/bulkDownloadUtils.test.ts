import {
  ...
  parseValidationInfo,
} from "../app/assets/src/components/views/DiscoveryView/components/SamplesView/components/BulkDownloadModal/utils.ts";
import { WorkflowRunStatusType } from "../app/assets/src/components/views/DiscoveryView/components/SamplesView/components/BulkDownloadModal/types.ts";
...
const runsOwnedByUser263 = {
  ...
  fedWorkflowRuns: nextgenValidationInfo.fedWorkflowRuns.filter(
...
describe("Bulk Download workflow validation", () => {
  ...
  test("nextgen parser works as expected with error", () => {
    const output = parseValidationInfo(
      { fedWorkflowRuns: undefined, error: ERROR_MESSAGE },
      selectedObjects,
    );
    expect(output).toStrictEqual(expectedWithError);
  });
