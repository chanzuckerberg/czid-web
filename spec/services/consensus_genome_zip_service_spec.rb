...
RSpec.describe ConsensusGenomeZipService, type: :service do
  ...
  describe "#generate" do
    it "presigns an S3 link for the target output zip" do
      ...
      expect(subject).to receive(:get_presigned_s3_url).with(s3_path: "fake_output_path", filename: "#{sample.name}_#{workflow_run.id}_outputs.zip").and_return(fake_presigned_link)
