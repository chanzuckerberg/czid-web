...
RSpec.describe SfnPipelineService, type: :service do
  ...
  context "generate_wdl_input" do
    ...
    it "contains fastq input files" do
      expect(subject).to include_json(
        Input: {
          HostFilter: JSON.dump(fastqs_0: format(SAMPLES_S3_PATH, sample_id: sample.id, project_id: project.id, input_file_name: sample.input_files[0].source),
    ...
    it "uploads per-stage dag json files" do
      pending "Needs test implementation"
    end
