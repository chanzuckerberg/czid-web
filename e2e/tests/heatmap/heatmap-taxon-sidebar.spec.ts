...
test.describe("Tests for Taxon Sidebar on Heatmap", () => {
  test.beforeEach(async ({ page }) => {
    // go to sample page
    await page.goto(
      `${process.env.BASEURL}/visualizations/heatmap?sampleIds[]=${sampleId1}&sampleIds[]=${sampleId2}`,
    );
    // click Taxon Name
    await page
      .getByTestId("row-label")
      .getByText("Klebsiella pneumoniae")
      .click();
  });
