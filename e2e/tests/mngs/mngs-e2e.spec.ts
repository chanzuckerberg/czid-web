...
import { IDSEQ_MOSQ_2TO4MIL_SUBSAMPLE_10P, HG002_LONG_READS_METAG, MWGS_SE_SRR7002140_TA_252_DNA_BLAC_VANP_10P, MWGS_PE_SRR7002140_TAP_R1, MWGS_PE_SRR7002140_TAP_R2, MWGS_RNA_MOSQUITO_1_AEDES_RNA_10p_R1, MWGS_RNA_MOSQUITO_1_AEDES_RNA_10p_R2, MWGS_RNA_HUMAN_128_LUNG_RNA_10p_R1, MWGS_RNA_HUMAN_128_LUNG_RNA_10p_R2 } from "@e2e/constants/sample";
...
const MWGS_PE_SRR7002140_TAP_FILES = [MWGS_PE_SRR7002140_TAP_R1, MWGS_PE_SRR7002140_TAP_R2];
...
const MWGS_RNA_MOSQUITO_FILES = [MWGS_RNA_MOSQUITO_1_AEDES_RNA_10p_R1, MWGS_RNA_MOSQUITO_1_AEDES_RNA_10p_R2];
const MWGS_RNA_MOSQUITO = "mWGS_RNA_mosquito-1-aedes-rna_10p";
const MWGS_RNA_MOSQUITO_SAMPLE_NAMES = [MWGS_RNA_MOSQUITO];

const MWGS_RNA_HUMAN_FILES = [MWGS_RNA_HUMAN_128_LUNG_RNA_10p_R1, MWGS_RNA_HUMAN_128_LUNG_RNA_10p_R2];
...
const DOWNLOAD_NON_HOST_CONTIGS_SUMMARY_CSV = "Download Non-Host Contigs Summary (.csv)";
...
const SAMPLE_DETAILS_SEQUENCING_PLATFORM = "Sample Details - Sequencing Platform";
const SAMPLE_DETAILS_PIPELINE_VERSION = "Sample Details - Pipeline Version";
const SAMPLE_DETAILS_GUPPY_BASECALLER_VERSION = "Sample Details - Guppy Basecaller Version";
...
const SAMPLE_DETAILS_PASSED_QUALITY_CONTROL = "Sample Details - Passed Quality Control";
...
const NO_DIFF_FIELDS = ["contig_name", "tax_id", "tax_level", "genus_tax_id", "name", "common_name", "category", "is_phage"];
...
test.describe("mNGS E2E | Functional: P-1", () => {
  ...
  test.skip("SNo e4: mNGS Nanopore Mosquito Sample Report & Download Data Validation", async ({ page }) => {
    ...
    collector.collect(async () => expect(row1["Taxon"]).toEqual("Acinetobacter( 3 bacterial species:1 )"), SAMPLE_REPORT_TAXON);

    // bMP=5,094.5 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["bPM"], [ "5,094.5", "0.0" ], SAMPLE_REPORT_BPM);

    // b=29,807 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["b"], [ "29,807", "0" ], SAMPLE_REPORT_B);

    // r=6 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["r"], [ "6", "0" ], SAMPLE_REPORT_R);

    // contig=0 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["contig"], [ "0", "0" ], SAMPLE_REPORT_CONTIG);

    // contig b=0 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["contig b"], [ "0", "0" ], SAMPLE_REPORT_CONTIG_B);

    // % id=93.4 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["%id"], [ "93.4", "0.0" ], SAMPLE_REPORT_PERCENTAGE_ID);

    // L=4,911.5 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["L"], [ "4,911.5", "0.0" ], SAMPLE_REPORT_L);

    // E value=10-308 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["E value"], [ "10-308", "0" ], SAMPLE_REPORT_E_VALUE);
    ...
    collector.collect(async () => expect(await samplesPage.getSampleDetailsAnalysisType()).toEqual("Metagenomic"), SAMPLE_DETAILS_ANALYSIS_TYPE);

    // Sequencing Platform=Nanopore
    collector.collect(async () => expect(await samplesPage.getSampleDetailsSequencingPlatform()).toEqual("Nanopore"), SAMPLE_DETAILS_SEQUENCING_PLATFORM);

    // Pipeline Version=v0.7 View Pipeline Visualization {VARIABLE VALUE}
    collector.collect(async () => expect(await samplesPage.getSampleDetailsPipelineVersion()).toEqual(PIPELINE_VISUALIZATION_7), SAMPLE_DETAILS_PIPELINE_VERSION);

    // Guppy Basecaller Version=hac
    collector.collect(async () => expect(await samplesPage.getSampleDetailsGuppyBasecallerVersion()).toEqual("hac"), SAMPLE_DETAILS_GUPPY_BASECALLER_VERSION);

    // NCBI Index Date=2021-01-22 {VARIABLE VALUE}
    collector.collect(async () => expect(await samplesPage.getSampleDetailsNcbiIndexDate()).toEqual(DATE_2021_01_22), SAMPLE_DETAILS_NCBI_INDEX_DATE);

    // Host Subtracted=Mosquito
    collector.collect(async () => expect(await samplesPage.getSampleDetailsHostSubtracted()).toEqual("Mosquito"), SAMPLE_DETAILS_HOST_SUBTRACTED);

    // Total Reads=2,000
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsTotalReads(), "2,000", SAMPLE_DETAILS_TOTAL_READS);

    // ERCC Reads=--
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsErccReads(), "--", SAMPLE_DETAILS_ERCC_READS);

    // Passed Filters=97 (4.85%)
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsPassedFilters(), "97 (4.85%)", SAMPLE_DETAILS_PASSED_FILTERS);

    // Unmapped Reads=64
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsUnmappedReads(), "64", SAMPLE_DETAILS_UNMAPPED_READS);

    // Passed Quality Control=99.95%
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsPassedQualityControl(), "99.95%", SAMPLE_DETAILS_PASSED_QUALITY_CONTROL);

    // Date Processed=2024-02-24 {VARIABLE VALUE}
    collector.collect(async () => expect(await samplesPage.getSampleDetailsDateProcessed()).toEqual(new Date().toISOString().substring(0, 10)), SAMPLE_DETAILS_DATE_PROCESSED);
    ...
    collector.collect(async () => expect(`${sampleName}_nonhost.fasta`).toEqual(nonHostReadsFasta.suggestedFilename()), DOWNLOAD_NON_HOST_READS_FASTA);
    ...
    collector.collect(async () => expect(`${sampleName}_contigs.fasta`).toEqual(nonHostContigsFasta.suggestedFilename()), DOWNLOAD_NON_HOST_CONTIGS_FASTA);
    downloadedContent.push(nonHostContigsFasta);

    // - Non-Host Contigs Summary (.csv)
    await samplesPage.clickDownloadButton();
    const nonHostContigsSummaryCsv = await samplesPage.clickDownloadOption(DOWNLOAD_NON_HOST_CONTIGS_SUMMARY_CSV);
    // - 28A-idseq-mosq.2to4mil_subsample_10p_contigs_summary.csv
    collector.collect(async () => expect(`${sampleName}_contigs_summary.csv`).toEqual(nonHostContigsSummaryCsv.suggestedFilename()), DOWNLOAD_NON_HOST_CONTIGS_SUMMARY_CSV);
    downloadedContent.push(nonHostContigsSummaryCsv);

    // - Unmapped Reads (.fasta)
    await samplesPage.clickDownloadButton();
    const unmappedReadsFasta = await samplesPage.clickDownloadOption(DOWNLOAD_UNMAPPED_READS_FASTA);
    // - 28A-idseq-mosq.2to4mil_subsample_10p_unidentified.fasta"
    collector.collect(async () => expect(`${sampleName}_unidentified.fasta`).toEqual(unmappedReadsFasta.suggestedFilename()), DOWNLOAD_UNMAPPED_READS_FASTA);
    ...
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/28A-idseq-mosq_2to4mil_subsample_10p/e4", sampleName, IDSEQ_MOSQ_2TO4MIL_SUBSAMPLE);
    // #endregion 6. From the sample report, click on the Download button for
  });

  test.skip("SNo e5: mNGS Nanopore Human Sample Report & Download Data Validation", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);
    ...
    collector.collect(async () => expect(row1["Taxon"]).toEqual("Lymphocryptovirus( 1 viral species:1 )"), SAMPLE_REPORT_TAXON);

    // bMP=189,668.0 | 137,520.0
    await verifyReportTableValuesUpperAndLowerBounds(row1["bPM"], [ "189,668.0", "137,520.0" ], SAMPLE_REPORT_BPM);

    // b=1,647,549 | 1,194,561
    await verifyReportTableValuesUpperAndLowerBounds(row1["b"], [ "1,647,549", "1,194,561" ], SAMPLE_REPORT_B);

    // r=130 | 115
    await verifyReportTableValuesUpperAndLowerBounds(row1["r"], [ "130", "115" ], SAMPLE_REPORT_R);

    // contig=3 | 3
    await verifyReportTableValuesUpperAndLowerBounds(row1["contig"], [ "3", "3" ], SAMPLE_REPORT_CONTIG);

    // contig b=1,194,561 | 1,194,561
    await verifyReportTableValuesUpperAndLowerBounds(row1["contig b"], [ "1,194,561", "1,194,561" ], SAMPLE_REPORT_CONTIG_B);

    // % id=99.4 | 99.4
    await verifyReportTableValuesUpperAndLowerBounds(row1["%id"], [ "99.4", "99.4" ], SAMPLE_REPORT_PERCENTAGE_ID);

    // L=52,176.6 | 1,954.5
    await verifyReportTableValuesUpperAndLowerBounds(row1["L"], [ "52,176.6", "1,954.5" ], SAMPLE_REPORT_L);

    // E value=10-308 | 10-304
    await verifyReportTableValuesUpperAndLowerBounds(row1["E value"], [ "10-308", "10-307" ], SAMPLE_REPORT_E_VALUE);
    // #endregion 4. Verify ""Taxon"" section data
    ...
    collector.collect(async () => expect(await samplesPage.getSampleDetailsHostSubtracted()).toEqual("Human"), SAMPLE_DETAILS_HOST_SUBTRACTED);

    // Total Reads=265
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsTotalReads(), "265", SAMPLE_DETAILS_TOTAL_READS);

    // ERCC Reads=--
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsErccReads(), "--", SAMPLE_DETAILS_ERCC_READS);

    // Passed Filters=163 (61.51%)
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsPassedFilters(), "163 (61.51%)", SAMPLE_DETAILS_PASSED_FILTERS);

    // Unmapped Reads=29
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsUnmappedReads(), "29", SAMPLE_DETAILS_UNMAPPED_READS);

    // Passed Quality Control=100.00%
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsPassedQualityControl(), "100.00%", SAMPLE_DETAILS_PASSED_QUALITY_CONTROL);
    ...
    // - Report table (.csv)
    await samplesPage.clickDownloadButton();
    const reportTableCsv = await samplesPage.clickDownloadOption("Download Report table (.csv)");
    ...
    // - Non-Host Reads (.fasta)
    await samplesPage.clickDownloadButton();
    const nonHostReadsFasta = await samplesPage.clickDownloadOption("Download Non-Host Reads (.fasta)");
    ...
    // - Non-Host Contigs (.fasta)
    await samplesPage.clickDownloadButton();
    const nonHostContigsFasta = await samplesPage.clickDownloadOption("Download Non-Host Contigs (.fasta)");
    ...
    // - Non-Host Contigs Summary (.csv)
    await samplesPage.clickDownloadButton();
    const nonHostContigsSummaryCsv = await samplesPage.clickDownloadOption("Download Non-Host Contigs Summary (.csv)");
    ...
    // - Unmapped Reads (.fasta)
    await samplesPage.clickDownloadButton();
    const unmappedReadsFasta = await samplesPage.clickDownloadOption("Download Unmapped Reads (.fasta)");
    ...
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/HG002_long_reads_metaG/e5", sampleName, HG002_LONG_READS_METAG_BASELINE_NAME);
    // #endregion 6. From the sample report, click on the Download button for:
  });

  test("SNo e6: mNGS Illumina Single Read Sample Report & Download Data Validation", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);
    ...
    for (const key of Object.keys(expectedReportTable)) {
      await verifyReportTableValuesUpperAndLowerBounds(row1[key], expectedReportTable[key], `Sample Report - ${key}`);
    }
    // #endregion 4. Verify ""Taxon"" section data
    ...
    for (const key of Object.keys(expectedPipelineInfoTable)) {
      await verifyBasesRemainingUpperAndLowerBounds(pipelineInfoTable[key], expectedPipelineInfoTable[key], READS);
    }
    ...
    collector.collect(async () => expect(readsRemainingTable.length).toEqual(expectedReadsRemainingTable.length), READS_REMAINING_COLUMN_COUNT);
    for (const i in expectedReadsRemainingTable) {
      await verifyBasesRemainingUpperAndLowerBounds(readsRemainingTable[i], expectedReadsRemainingTable[i], READS);
    }
    ...
    // project 1151
    // samples 48440
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/mWGS_SE_SRR7002140_TA/e6", sampleName, MWGS_SE_SAMPLE);
    // #endregion 7. Verify the specified data file outputs against baseline outputs"
  });

  test("SNo e7: mNGS Illumina Paired Read Sample Report & Download Data Validation", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);
    ...
    // project 1151
    // samples 50118
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/mWGS_PE_SRR7002140_TA/e7", sampleName, MWGS_PE_SRR7002140_TAP);
    // #endregion 7. Verify the specified data file outputs against baseline outputs"
  });

  test("SNo e8: mNGS Illumina Paired Read RNA Mosquito Sample Report & Download Data Validation", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);
    ...
    // project 1151
    // samples 50120
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/mWGS_RNA_MOSQUITO_1_AEDES_RNA_10p/e8", sampleName, MWGS_RNA_MOSQUITO);
    // #endregion 7. Verify the specified data file outputs against baseline outputs"
  });

  test("SNo e9: mNGS Illumina Paired Read RNA Human Sample Report & Download Data Validation", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);
    ...
    // project 1151
    // samples 50124
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/mWGS_RNA_HUMAN_128_LUNG_RNA_10p/e9", sampleName, MWGS_RNA_HUMAN);
    // #endregion 7. Verify the specified data file outputs against baseline outputs
  });

  test.skip("SNo e14: mNGS Nanopore Human Sample Report & Download Data Validation - New database", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);

    // #region 1. Login to CZ ID staging
    const projectPage = new ProjectPage(page);
    const project = await projectPage.getOrCreateProject(`SNo-e14_${WORKFLOWS.MNGS}`);
    // #endregion 1. Login to CZ ID staging
    ...
    const row1 = reportTable[0];
    // Taxon=Lymphocryptovirus( 1 viral species:1 )
    collector.collect(async () => expect(row1["Taxon"]).toEqual("Lymphocryptovirus( 1 viral species )"), SAMPLE_REPORT_TAXON);
    ...
    // % id=92.6 | 98.9
    await verifyReportTableValuesUpperAndLowerBounds(row1["%id"], [ "92.6", "98.9" ], SAMPLE_REPORT_PERCENTAGE_ID);

    // L=63,570.0 | 1,954.5
    await verifyReportTableValuesUpperAndLowerBounds(row1["L"], [ "63,570.0", "1,954.5" ], SAMPLE_REPORT_L);

    // E value=10-308 | 10-304
    await verifyReportTableValuesUpperAndLowerBounds(row1["E value"], [ "10-308", "10-304" ], SAMPLE_REPORT_E_VALUE);
    // #endregion 4. Verify ""Taxon"" section data
    ...
    // NCBI Index Date=2021-01-22 {VARIABLE VALUE}
    collector.collect(async () => expect(await samplesPage.getSampleDetailsNcbiIndexDate()).toEqual(DATE_2024_02_06), SAMPLE_DETAILS_NCBI_INDEX_DATE);
    ...
    // Unmapped Reads=29
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsUnmappedReads(), "30", SAMPLE_DETAILS_UNMAPPED_READS);
    ...
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/HG002_long_reads_metaG/e14", sampleName, HG002_LONG_READS_METAG_BASELINE_NAME);
    // #endregion 6. From the sample report, click on the Download button for:
  });

  test.skip("SNo e15: mNGS Nanopore Mosquito Sample Report & Download Data Validation  - New database", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);

    // #region 1. Login to CZ ID staging
    const projectPage = new ProjectPage(page);
    const project = await projectPage.getOrCreateProject(`SNo-e15_${WORKFLOWS.MNGS}`);
    // #endregion 1. Login to CZ ID staging
    ...
    const row1 = reportTable[0];
    // Taxon=Acinetobacter( 3 bacterial species:1 )
    collector.collect(async () => expect(row1["Taxon"]).toEqual("Acinetobacter( 2 bacterial species )"), SAMPLE_REPORT_TAXON); // Expected: Taxon=Acinetobacter( 3 bacterial species:1 ), Actual: Acinetobacter( 2 bacterial species )
    ...
    // % id=93.9 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["%id"], [ "93.9", "0.0" ], SAMPLE_REPORT_PERCENTAGE_ID);

    // L=4,910.0 | 0
    await verifyReportTableValuesUpperAndLowerBounds(row1["L"], [ "4,910.0", "0.0" ], SAMPLE_REPORT_L);
    ...
    // Unmapped Reads=58
    await verifyPipelineUpperAndLowerBounds(await samplesPage.getSampleDetailsUnmappedReads(), "58", SAMPLE_DETAILS_UNMAPPED_READS);
    ...
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/28A-idseq-mosq_2to4mil_subsample_10p/e15", sampleName, IDSEQ_MOSQ_2TO4MIL_SUBSAMPLE);
    // #endregion 6. From the sample report, click on the Download button for:
  });

  test("SNo e16: mNGS Illumina Single Read Sample Report & Download Data Validation - New database", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);

    // #region 1. Login to CZ ID staging
    const projectPage = new ProjectPage(page);
    const project = await projectPage.getOrCreateProject(`SNo-e16_${WORKFLOWS.MNGS}`);
    // #endregion 1. Login to CZ ID staging
    ...
    // project 1479
    // samples 50056
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/mWGS_SE_SRR7002140_TA/e16", sampleName, MWGS_SE_SAMPLE);
    // #endregion 7. Verify the specified data file outputs against baseline outputs"
  });

  test("SNo e17: mNGS Illumina Paired Read Sample Report & Download Data Validation - New database", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);

    // #region 1. Login to CZ ID staging
    const projectPage = new ProjectPage(page);
    const project = await projectPage.getOrCreateProject(`SNo-e17_${WORKFLOWS.MNGS}`);
    // #endregion 1. Login to CZ ID staging
    ...
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/mWGS_PE_SRR7002140_TA/e17", sampleName, MWGS_PE_SRR7002140_TAP);
    // #endregion 7. Verify the specified data file outputs against baseline outputs"
  });

  test("SNo e18: mNGS Illumina Paired Read RNA Mosquito Sample Report & Download Data Validation - New database", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);

    // #region 1. Login to CZ ID staging
    const projectPage = new ProjectPage(page);
    const project = await projectPage.getOrCreateProject(`SNo-e18_${WORKFLOWS.MNGS}`);
    // #endregion 1. Login to CZ ID staging
    ...
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/mWGS_RNA_MOSQUITO_1_AEDES_RNA_10p/e18", sampleName, MWGS_RNA_MOSQUITO);
    // #endregion 7. Verify the specified data file outputs against baseline outputs"
  });

  test("SNo e19: mNGS Illumina Paired Read RNA Human Sample Report & Download Data Validation - New database", async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);
    // #region 1. Login to CZ ID staging
    const projectPage = new ProjectPage(page);
    const project = await projectPage.getOrCreateProject(`SNo-e19_${WORKFLOWS.MNGS}`);
    // #endregion 1. Login to CZ ID staging
    ...
    await compareDataFilesWithTolerance(downloadedContent, "mNGS/mWGS_RNA_HUMAN_128_LUNG_RNA_10p/e19", sampleName, MWGS_RNA_HUMAN);
  });
});
...
async function verifyPipelineUpperAndLowerBounds(actual: any, expected: any, message = "") {
  ...
  if (expected.match(numberAndPercentage) || expected.match(mean)) {
    ...
    await verifyUpperAndLowerBounds(
      ...
      await parseFloatFromFormattedNumber(expectedNumberStr1), message,
    );
    await verifyUpperAndLowerBounds(
      await parseFloatFromFormattedNumber(actualNumberStr2),
      await parseFloatFromFormattedNumber(expectedNumberStr2), message,
    );
    ...
    await verifyUpperAndLowerBounds(
      ...
      await parseFloatFromFormattedNumber(expectedNumberStr), message,
    );
    ...
    await verifyUpperAndLowerBounds(
      ...
      await parseFloatFromFormattedNumber(expected), message,
    );
  }
}

async function verifyReportTableValuesUpperAndLowerBounds(actualReportTableValues: any, expectedReportTableValues: any, message = "") {
  const eValueRegex = /(\d+)-(\d+)/;
  if (message.endsWith("Taxon")) {
    collector.collect(async () => expect(actualReportTableValues).toMatch(expectedReportTableValues), message);
  } else if (!Array.isArray(expectedReportTableValues) || !Array.isArray(actualReportTableValues)) {
    collector.collect(async () => expect(actualReportTableValues).toEqual(expectedReportTableValues), message); // "-"
  } else if (expectedReportTableValues[0].match(eValueRegex) || expectedReportTableValues[1].match(eValueRegex)) {
    ...
    for (const i in expectedReportTableValues) {
      ...
      if (eValueMatch) {
        ...
        await verifyUpperAndLowerBounds(actualEValue1, expectedEValue1, message);
        await verifyUpperAndLowerBounds(actualEValue2, expectedEValue2, message);
      } else {
        collector.collect(async () => expect(actualReportTableValues[i]).toEqual(expectedReportTableValues[i]), message); // "0"
      }
    }
  } else {
    const expected1 = await parseFloatFromFormattedNumber(expectedReportTableValues[0]);
    const expected2 = await parseFloatFromFormattedNumber(expectedReportTableValues[1]);
    const actual1 = await parseFloatFromFormattedNumber(actualReportTableValues[0]);
    const actual2 = await parseFloatFromFormattedNumber(actualReportTableValues[1]);
...
async function verifyBasesRemainingUpperAndLowerBounds(actualBasesRemaining: any, expectedBasesRemaining: any, tableKey = "Bases") {
  const filteringStep = actualBasesRemaining["Filtering Step"];
  collector.collect(async () => expect(filteringStep).toEqual(expectedBasesRemaining["Filtering Step"]), "Sample Details - Filtering Step");
  ...
  if (Array.isArray(expectedBasesRemaining[remainingKey])) {
    if (!Array.isArray(actualBasesRemaining[remainingKey])) {
      collector.collect(async () => expect(actualBasesRemaining[remainingKey]).toEqual(expectedBasesRemaining[remainingKey]), message);
    } else {
      const basesRemaining = await parseIntFromFormattedNumber(actualBasesRemaining[remainingKey][0]);
      const basesRemainingUnique = await parseIntFromUniqueNumber(actualBasesRemaining[remainingKey][1]);

      const expectedBasesRemainingValue = await parseIntFromFormattedNumber(expectedBasesRemaining[remainingKey][0]);
      await verifyUpperAndLowerBounds(basesRemaining, expectedBasesRemainingValue, message);

      const expectedBasesRemainingUniqueValue = await parseIntFromUniqueNumber(expectedBasesRemaining[remainingKey][1]);
      await verifyUpperAndLowerBounds(basesRemainingUnique, expectedBasesRemainingUniqueValue, message);
    }
  } else if (typeof expectedBasesRemaining[remainingKey] === "string") {
    if (actualBasesRemaining[remainingKey].includes("%")) {
      const percentageBasesRemaining = await parseFloatFromPercentage(actualBasesRemaining[percentageKey]);
      const expectedPercentageBasesRemaining = await parseFloatFromPercentage(expectedBasesRemaining[percentageKey]);
      await verifyUpperAndLowerBounds(percentageBasesRemaining, expectedPercentageBasesRemaining, message);
    } else if (expectedBasesRemaining[remainingKey].match(/[\d,]+/)) {
      const basesRemaining = await parseIntFromFormattedNumber(actualBasesRemaining[remainingKey]);
      const expectedBasesRemainingValue = await parseIntFromFormattedNumber(expectedBasesRemaining[remainingKey]);
      await verifyUpperAndLowerBounds(basesRemaining, expectedBasesRemainingValue, message);
    } else {
      collector.collect(async () => expect(actualBasesRemaining[remainingKey]).toEqual(expectedBasesRemaining[remainingKey]), message);
    }
...
async function verifyUpperAndLowerBounds(actual: any, expected: any, message = "") {
  const expectedRange = await getUpperAndLowerBounds(expected);
  const msg = `${message} ${actual} vs ${expected}: Diff ${Math.abs((actual - expected) / ((actual + expected) / 2)) * 100}`;
  collector.collect(async () => expect(actual).toBeGreaterThanOrEqual(expectedRange.lowerBound), msg);
  collector.collect(async () => expect(actual).toBeLessThanOrEqual(expectedRange.upperBound), msg);
}
...
async function compareCSV(contentName: string, actualOutputData: any, expectedBaselineData: any) {
  ...
  const sortKey = contentName.endsWith("_summary.csv") ? "contig_name" : "tax_id";
  ...
  for (const i in expectedData) {
    ...
    for (const key of Object.keys(expectedRow)) {
      ...
      if (NO_DIFF_FIELDS.includes(key) || !expectedValue || !actualValue) {
        collector.collect(async () => expect(actualValue).toEqual(expectedValue), message + key);
        ...
        if (!actualValue.startsWith("[")) {
          ...
          for (const i in expectedArray) {
            await verifyUpperAndLowerBounds(
              ...
              parseFloat(expectedArray[i]), message + key,
            );
          }
        }
      } else {
        await verifyUpperAndLowerBounds(parseFloat(actualValue), parseFloat(expectedValue) , message + key);
      }
    }
  }
}
