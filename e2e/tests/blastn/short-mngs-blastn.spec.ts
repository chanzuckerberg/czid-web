...
test.describe("Functional: P-1: short mNGS - BLASTN", () => {
   ...
   */
  test(`SNo 22: To verify New Index BLASTN functionality sample report`, async ({ page }) => {
    ...
    const project = await projectPage.getOrCreateProject("SNo_22-NewIndex-BLASTN");
    await setupSamples(
      page,
      project,
      MNGS_SAMPLE_FILES,
   ...
   */
  test(`SNo 24: To verify Old Index BLASTN functionality sample report`, async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);
    ...
    const taxons = await samplesPage.getTaxonsFromReport(
      await samplesPage.getReportV2(sample.id));
    const taxon = taxons.filter(taxon => taxon.name && taxon.name.includes("Human orthopneumovirus"))[0]
    await samplesPage.hoverOverTaxon(taxon.name);
    await samplesPage.pause(2);
    // #endregion 4. Expand genus / species list and hover over a species record
