...
test.describe("Functional: P-1: short mNGS - Coverage Visualization", () => {
   ...
   */
  test(`SNo 30: To verify New Index NCBI Refernece Coverage Vis functionality, NCBI reference IDs, contig files download in a sample report with the new index`, async ({
    page,
  }) => {
    ...
    const taxon = taxons.filter(
      taxon => reportTable[1].Taxon.includes(taxon.name) && taxon.name,
    )[0];
   ...
   */
  test(`SNo 31: To verify Old Index NCBI Refernece Coverage Vis functionality, NCBI reference IDs, contig files download in a sample report`, async ({
    page,
  }) => {
    test.setTimeout(TEST_TIMEOUT);
