...
test.describe("Sample upload (web): Functional: P-0", () => {
  ...
  for (const wetLab of ILLUMINA_WET_LABS) {
    ...
    */
    test(`SNo SC2-1: SARS-CoV-2 Illumina sample web upload with wetlab ${wetLab}`, async ({ page }) => {
      ...
      const sampleStatusMessage = await samplesPage.getSampleStatusMessage(IN_PROGRESS);
      ...
      for (const sampleName of sampleNames) {
        expect(samplesTable[sampleName][REFERENCE_ACCESSION]).toEqual(EXPECTED_REFERENCE_ACCESSION);
      }
  ...
  Clear Labs
  */
  test(`SNo SC2-3: SARS-CoV-2 Nanopore sample web upload (ARTIC v3 - r941_min_high_g360)`, async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);
    ...
    const actualMedakaModelDescription = await uploadPage.getMedakaModelDescription();
    ...
    expect(actualMedakaModelDescription).toEqual(expectedMedakaModelDescription);
    ...
    let inputs = await uploadPage.getRandomizedSampleInputs([SAMPLE_FILE_CT20K], [CT20K_SAMPLE_NAME]);
    inputs = await uploadPage.setManualInputs(inputs);
    await uploadPage.clickContinue();
    ...
    for (const sampleName of sampleNames) {
      ...
      expect(samplesTable[sampleName][MEDAKA_MODEL]).toEqual("r941_min_high_g360");
    }
  ...
  Midnight
  */
  test(`SNo SC2-4: SARS-CoV-2 Nanopore sample web upload (Midnight - Medaka Model)`, async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT);
...
test.describe("Sample upload (web) - SC2  Illumina: Functional: P-1", () => {
  ...
  test(`SNo SC2-2: Verify Sample upload process LINKS`, async ({ page }) => {
    ...
    expect(await linkedPage.url()).toEqual("https://github.com/chanzuckerberg/czid-workflows/tree/main/workflows/consensus-genome");
    ...
    expect(await linkedPage.url()).toEqual("https://chanzuckerberg.zendesk.com/hc/en-us/articles/360059656311-Upload-Data-and-Assemble-SARS-CoV-2-Genomes-Using-the-Web-App");
    await linkedPage.close();
    ...
    expect(await linkedPage.url()).toEqual("https://artic.network/ncov-2019/ncov2019-bioinformatics-sop.html");
    await linkedPage.close();
    ...
    expect(columnTooltip).toEqual("Pipeline will be adjusted to accomodate Clear Lab fastq files which have undergone the length filtering and trimming steps. Learn more.");
    ...
    expect(columnTooltip).toEqual("For best results, specify the correct model. Where a version of Guppy has been used without a corresponding model, choose a model with the highest version equal to or less than the Guppy version. Learn more.");
    ...
    expect(await linkedPage.url()).toEqual("https://chanzuckerberg.zendesk.com/hc/en-us/articles/360059656311-Upload-Data-and-Assemble-SARS-CoV-2-Genomes-Using-the-Web-App#Upload");
