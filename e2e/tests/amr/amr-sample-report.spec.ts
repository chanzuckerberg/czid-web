...
const ADDITIONAL_CONTIGS_COLUMNS_ON_EXPAND = ["Contig SpeciesBETA"];
...
const ADDITIONAL_READS_COLUMNS_ON_EXPAND = ["Read SpeciesBETA"];
...
const verifyExpectedColumnsChange = (
  expectedColumnChanges: string[],
  actualColumns: string[],
  columnIncluded = true,
) =>
  expectedColumnChanges.every(
    column => actualColumns.includes(column) === columnIncluded,
  );
...
test.describe(`AMR Sample Report tests for ${E_COLI_CFXA_SAMPLE_NAME}`, () => {
  ...
  test.describe("Sample report table", () => {
    test("SNo 6: Verify cfxA gene is in report", async () => {
      ...
      const cfxaGenePresent =
        reportTable.findIndex(geneRow => geneRow["Gene"] === CFXA_GENE) >= 0;
    ...
    test("SNo 7: Drug class filter", async () => {
      ...
      const reportTableRowsBefore = (
        await samplesPage.getAntimicrobialResistanceTable(2, 9)
      ).length;
      ...
      const reportTableAfter =
        await samplesPage.getAntimicrobialResistanceTable(2, 3);
    ...
    test("SNo 9: Verify expanded and collapse contigs and reads columns", async () => {
      ...
      expect(
        verifyExpectedColumnsChange(
          ADDITIONAL_CONTIGS_COLUMNS_ON_EXPAND,
          expandedContigColumns,
          true,
        ),
      ).toBeTruthy();
      ...
      expect(
        verifyExpectedColumnsChange(
          REMOVED_CONTIGS_COLUMNS_ON_COLLAPSE,
          collapsedContigColumns,
          false,
        ),
      ).toBeTruthy();
      ...
      expect(
        verifyExpectedColumnsChange(
          ADDITIONAL_READS_COLUMNS_ON_EXPAND,
          expandedReadsColumns,
          true,
        ),
      ).toBeTruthy();
      ...
      expect(
        verifyExpectedColumnsChange(
          REMOVED_READS_COLUMNS_ON_COLLAPSE,
          collapsedReadsColumns,
          false,
        ),
      ).toBeTruthy();
    ...
    test("SNo 10: Verify sample report table sorting", async () => {
      ...
      const descendingSortedGeneNames = (
        await samplesPage.getAntimicrobialResistanceTable(2, 9)
      ).map(row => row["Gene"]);
      ...
      for (let i = 1; i < descendingSortedGeneNames.length; i++) {
        if (
          descendingSortedGeneNames[i].localeCompare(
            descendingSortedGeneNames[i - 1],
          ) >= 0
        ) {
      ...
      await samplesPage.clickAmrTableGeneColumn();
      const ascendingSortedGeneNames = (
        await samplesPage.getAntimicrobialResistanceTable(2, 9)
      ).map(row => row["Gene"]);
      // #endregion Sort by gene and get gene names from table
      ...
      for (let i = 1; i < ascendingSortedGeneNames.length; i++) {
        if (
          ascendingSortedGeneNames[i].localeCompare(
            ascendingSortedGeneNames[i - 1],
          ) <= 0
        ) {
    ...
    test("SNo 11: Verify Reads filter", async () => {
      // #region Get initial full report table
      const reportTableNumRowsBefore = (
        await samplesPage.getAntimicrobialResistanceTable(2, 9)
      ).length;
      // #endregion Get initial full report table

      // #region Apply reads filter
      await samplesPage.selectAmrThresholdOption(
        THRESHOLD_FILTERS_NUMBER_OF_READS,
        THRESHOLD_VALUE_NUM_READS,
      );
      // #endregion Apply reads filter

      // #region Validate reads filter reduces number of rows and each row meets threshold
      const reportTableRowsAfter =
        await samplesPage.getAntimicrobialResistanceTable(2, 9);
      expect(reportTableRowsAfter.length).toBeLessThan(
        reportTableNumRowsBefore,
      );

      const allRowsMeetThreshold = reportTableRowsAfter.every(
        row => row["Reads"] >= THRESHOLD_VALUE_NUM_READS,
      );
    ...
    test("SNo 20: Verify sample report persistence", async () => {
      ...
      const initialFirstGeneRow = (
        await samplesPage.getAntimicrobialResistanceTable(2, 1)
      )[0];
      ...
      await samplesPage.clickAmrTableContigsHeader();
      const firstGeneRowContigsExpanded = (
        await samplesPage.getAntimicrobialResistanceTable(2, 1)
      )[0];
      const columnHeadersContigsExpanded = Object.keys(
        firstGeneRowContigsExpanded,
      );
      expect(
        verifyExpectedColumnsChange(
          ADDITIONAL_CONTIGS_COLUMNS_ON_EXPAND,
          columnHeadersContigsExpanded,
          true,
        ),
      ).toBeTruthy();
      ...
      await samplesPage.reload();
      const firstGeneRowContigsExpandedReload = (
        await samplesPage.getAntimicrobialResistanceTable(2, 1)
      )[0];
      const columnHeadersContigsExpandedReload = Object.keys(
        firstGeneRowContigsExpandedReload,
      );
      expect(
        verifyExpectedColumnsChange(
          ADDITIONAL_CONTIGS_COLUMNS_ON_EXPAND,
          columnHeadersContigsExpandedReload,
          true,
        ),
      ).toBeTruthy();
  ...
  test.describe("Sample report downloads", () => {
    test("SNo 12: Verify reads per gene download", async () => {
      ...
      const downloadNumSequences = await numSequencesInDownloadFasta(
        marAGeneReadsFastaDownload,
      );
      const expectedNumSequences = await numSequencesInDownloadFixtureFasta(
        marAGeneReadsFastaDownload,
        FIXTURE_PATH,
      );
    ...
    test("SNo 14: Verify report table CSV download", async () => {
      ...
      const reportTableDownload = await samplesPage.clickDownloadOption(
        DOWNLOAD_TYPES_OPTIONS_MAP["report_table"],
      );
    ...
    test("SNo 15: Verify report table with filters CSV download", async () => {
      ...
      await samplesPage.clickDownloadButton();
      const reportTableDownload = await samplesPage.clickDownloadOption(
        DOWNLOAD_TYPES_OPTIONS_MAP["report_table_filters"],
      );
      // #endregion download non-host reads file
    ...
    test("SNo 16: Verify sample non-host reads download", async () => {
      ...
      await samplesPage.clickDownloadButton();
      const nonHostReadsFastaDownload = await samplesPage.clickDownloadOption(
        DOWNLOAD_TYPES_OPTIONS_MAP["non_host_reads"],
      );
      // #endregion download non-host reads file

      // #region Verify number of sequences in downloaded file
      const nonHostReadsNumSequences = await numSequencesInDownloadFasta(
        nonHostReadsFastaDownload,
      );
      const maximumSequenceCountDiff = Math.ceil(
        EXPECTED_NUMBER_OF_SEQUENCES * sequenceCountTolerance,
      );
      const sequenceCountDiff = Math.abs(
        nonHostReadsNumSequences - EXPECTED_NUMBER_OF_SEQUENCES,
      );
    ...
    test("SNo 17: Verify sample non-host contigs download", async () => {
      ...
      await samplesPage.clickDownloadButton();
      const nonHostContigsFastaDownload = await samplesPage.clickDownloadOption(
        DOWNLOAD_TYPES_OPTIONS_MAP["non_host_contigs"],
      );
      // #endregion download non-host contigs file

      // #region Verify number of sequences in downloaded file
      const downloadNumSequences = await numSequencesInDownloadFasta(
        nonHostContigsFastaDownload,
      );
      const expectedNumSequences = await numSequencesInDownloadFixtureFasta(
        nonHostContigsFastaDownload,
        FIXTURE_PATH,
      ...
      const maximumSequenceCountDiff = Math.ceil(
        expectedNumSequences * sequenceCountTolerance,
      );
      const sequenceCountDiff = Math.abs(
        downloadNumSequences - expectedNumSequences,
      );
    ...
    test("SNo 18: Verify comprehensive AMR metrics TSV download", async () => {
      ...
      await samplesPage.clickDownloadButton();
      const comprehensiveAmrMetricsDownload =
        await samplesPage.clickDownloadOption(
          DOWNLOAD_TYPES_OPTIONS_MAP["comprehensive_amr_metrics"],
        );
      ...
      const downloadName =
        await comprehensiveAmrMetricsDownload.suggestedFilename();

      const actualNumberOfRows = await getNumberOfLinesInFile(downloadPath);

      const downloadfixturePath = getFixturePathForSampleDownload(
        downloadName,
        FIXTURE_PATH,
        sampleName,
        E_COLI_CFXA_SAMPLE_NAME,
      );
      const expectedNumberOfRows = await getNumberOfLinesInFile(
        downloadfixturePath,
      );
    ...
    test("SNo 19: Verify Intermediate Files zip download", async () => {
      ...
      await samplesPage.clickDownloadButton();
      const intermediateFilesZipDownload =
        await samplesPage.clickDownloadOption(
          DOWNLOAD_TYPES_OPTIONS_MAP["intermediate_files"],
        );
