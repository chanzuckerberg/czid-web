...
const SAMPLES_IN_DOWNLOAD_NAMES =
  "[class*='samplesList'] [class*='sampleName']";
...
const DOWNLOAD_DETAILS = (downloadId: string) =>
  `[id='${downloadId}'][data-testid='download-details-link']`;
const DOWNLOAD_COMPLETE_BY_DOWNLOADID = (downloadId: string) =>
  `//div[contains(@data-testid, 'complete')]/parent::div/following-sibling::div/span[@id='${downloadId}']`;
const DOWNLOAD_FILE_BY_DOWNLOADID = (downloadId: string) =>
  `//div[@id='${downloadId}' and text()='Download File']`;
const DOWNLOAD_DATE_BY_DOWNLOADID = (downloadId: string) =>
  `//div[@id='${downloadId}']/ancestor::div[contains(@class, '__Table__row tableRow-')]//div[@data-testid='date-created']`;
const DOWNLOAD_NAME_BY_DOWNLOADID = (downloadId: string) =>
  `//div[@id='${downloadId}']/ancestor::div[contains(@class, '__Table__row tableRow-')]//div[@data-testid='download-name']`;
const DOWNLOAD_COUNT_BY_DOWNLOADID = (downloadId: string) =>
  `//div[@id='${downloadId}']/ancestor::div[contains(@class, '__Table__row tableRow-')]//div[@aria-colindex='3']/div`;
const DOWNLOAD_STATUS_BY_DOWNLOADID = (downloadId: string) =>
  `//div[contains(@class, 'downloadStatus')]/parent::div/following-sibling::div/span[@id='${downloadId}']`;
const DOWNLOAD_STATUS_BY_INDEX = (rowIndex: string) =>
  `(//*[contains(@class, 'downloadStatus')])[${rowIndex}]`;
...
export class DownloadsPage extends PageObject {
  ...
  public async getDownloadCount(downloadId: string) {
    return (
      await this.page
        .locator(DOWNLOAD_COUNT_BY_DOWNLOADID(downloadId))
        .textContent()
    ).split(" ")[0];
  }
  ...
  public async getDownloadName(downloadId: string) {
    return this.page
      .locator(DOWNLOAD_NAME_BY_DOWNLOADID(downloadId))
      .textContent();
  }

  public async getDownloadDate(downloadId: string) {
    return this.page
      .locator(DOWNLOAD_DATE_BY_DOWNLOADID(downloadId))
      .textContent();
  }
  ...
  public async getDownloadTypeCount(downloadType: string) {
    const downloadTypes = await this.page
      .locator(DOWNLOAD_DETAILS(downloadType))
      .all();
  ...
  public async waitForDownloadComplete(downloadId: number, timeout = 30000) {
    ...
    while (Date.now() - startTime < timeout) {
      complete = await this.page
        .locator(DOWNLOAD_COMPLETE_BY_DOWNLOADID(downloadId.toString()))
        .isVisible();
      if (!complete) {
  ...
  public async isDownloadVisible(downloadId: number) {
    await this.page
      .locator(DOWNLOAD_STATUS_BY_DOWNLOADID(downloadId.toString()))
      .isVisible();
  }

  // #endregion Macro

  public async downloadSmokeTest(
    workflow: string,
    downloadType: string,
    timeout: number,
  ) {
    ...
    if (workflow === "mngs" && downloadType === "Sample Taxon Reports") {
      ...
      bulkDownloadMetric =
        bulkDownloadMetrics[
          Math.floor(Math.random() * bulkDownloadMetrics.length)
        ];
      ...
      bulkDownloadMetric =
        downloadMetrics[Math.floor(Math.random() * downloadMetrics.length)];
      ...
      const fileFormat =
        fileFormats[Math.floor(Math.random() * fileFormats.length)];
      ...
      await projectPage.clickFilterDropdown();
      const option = ["Separate Files", "Single File (Concatenated)"][
      ...
      downloadType === "Consensus Genome Overview" &&
    ...
    if (
      downloadType.includes("Consensus Genome Overview") ||
      ...
      expect(`${fileName}.${expectedFileExtention}`).toMatch(
    ...
    const downloadComplete = await this.waitForDownloadComplete(
      downloadId,
      timeout,
    );
    ...
    if (
      downloadType === "Intermediate Output Files" &&
      (await this.isFeatureFlagUser())
    ) {
  ...
  public async downloadCombinedMicrobiomeFileDownload(
    biomDownloadId,
    testTimeout = 120000,
  ) {
