module BenchmarksHelper
  ...
  def load_benchmarks_config
    ...
    LogUtil.log_error("Config file not found - '#{IDSEQ_BENCH_KEY}'")
    return nil
  rescue JSON::ParserError
    LogUtil.log_error("Invalid config file - '#{IDSEQ_BENCH_KEY}'")
    return nil
  rescue StandardError => e
    LogUtil.log_error("Unknown error - '#{e.message}'", exception: e)
    return nil
  end
