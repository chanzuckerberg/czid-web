...
class Sample < ApplicationRecord
  ...
  def self.get_signed_url(s3key)
    begin
      ...
      LogHelper.log_err_and_airbrake("AWS presign error: #{e.inspect}")
    end
    nil
  end
  ...
  def initiate_s3_cp
    ...
    unless stderr_array.empty?
      LogHelper.log_err_and_airbrake("Failed to upload sample #{id} with error #{stderr_array[0]}")
  ...
  def concatenate_input_parts
    ...
    begin
      ...
      LogHelper.log_err_and_airbrake("Failed to concatenate input parts for sample #{id}")
    end
  end
