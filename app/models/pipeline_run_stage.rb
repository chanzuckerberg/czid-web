class PipelineRunStage < ApplicationRecord
  ...
  def update_job_status
    if !id || !started?
      LogHelper.log_err_and_airbrake("Invalid precondition for PipelineRunStage.update_job_status #{id} #{job_id} #{job_status}.")
      return
    end
    ...
    unless count_failed_tries <= MAX_RETRIES
      LogHelper.log_err_and_airbrake("Job #{job_id} for pipeline run #{id} was killed #{MAX_RETRIES} times.")
      save
      return
    end
    run_job # this saves
