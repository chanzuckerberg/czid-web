...
class Samples extends React.Component {
  ...
  selectTissueFilter(e) {
    ...
    const selectedFilter = e.target.getAttribute('data-status');
  ...
  checkReportDownload() {
    axios.get(`/projects/${this.state.project_id_download_in_progress}/project_reports_csv_status`).then((res) => {
      const download_status = res.data.status_display;
  ...
  updateUserDisplay(email_to_add) {
    const new_project_users = this.state.project_users;
  ...
  handleAddUser(e, waitForEnter) {
    if (waitForEnter && e.keyCode !== 13) {
      ...
      const email_to_add = this.refs.add_user.value;
      const project_id = this.state.selectedProjectId;
  ...
  appendStatusIcon(status) {
    switch (status) {
      case 'WAITING':
        return <i className="waiting fa fa-arrow-up" aria-hidden="true" />;
        break;
      case 'INPROGRESS':
        return <i className="uploading fa fa-repeat" aria-hidden="true" />;
        break;
      ...
      case 'ALIGNMENT':
        ...
        break;
      case 'FAILED':
        return <i className="failed fa fa-times" aria-hidden="true" />;
        break;
      case 'COMPLETE':
        return <i className="complete fa fa-check" aria-hidden="true" />;
        break;
      default:
        return <i className="waiting fa fa-arrow-up" aria-hidden="true" />;
    }
  ...
  renderPipelineOutput(samples) {
    const BLANK_TEXT = <span className="blank">NA</span>;
    return samples.map((sample, i) => {
      const dbSample = sample.db_sample;
      const derivedOutput = sample.derived_sample_output;
      const runInfo = sample.run_info;
      const uploader = sample.uploader.name;
      const statusClass = !runInfo.job_status_description ? this.applyChunkStatusClass(runInfo) : this.applyClass(runInfo.job_status_description);
      const status = !runInfo.job_status_description ? this.getChunkedStage(runInfo) : runInfo.job_status_description;
      ...
      const data_values = {
        total_reads: !derivedOutput.pipeline_run ? BLANK_TEXT : numberWithCommas(derivedOutput.pipeline_run.total_reads),
              ...
              <div className="flex-container">
                <ul className="flex-items">
                  <li className="check-box-container">
                    { this.state.displaySelectSamplees ? <div><input
                      type="checkbox"
                      id={i}
                      onClick={this.selectSample}
                      className="filled-in checkbox"
                      value={this.state.selectedSampleIndices.indexOf(i) != -1}
                      disabled={status != 'COMPLETE'}
                    /> <label htmlFor={i}>{sample_name_info}</label>
                    </div> : sample_name_info }
                  </li>
                  {
  ...
  getParams() {
    ...
    if (this.state.sampleIdsParams.length) {
      const sampleParams = this.state.sampleIdsParams;
      params += `&ids=${sampleParams}`;
    }

    if (this.state.selectedTissueFilters.length) {
      const tissueParams = this.state.selectedTissueFilters.join(',');
    ...
    if (this.state.selectedHostIndices.length) {
      const hostParams = this.state.selectedHostIndices.join(',');
  ...
  applyChunkStatusClass(runInfo) {
    const postProcess = runInfo['Post Processing'];
    const hostFiltering = runInfo['Host Filtering'];
    const alignment = runInfo['GSNAPL/RAPSEARCH alignment'];
    if (postProcess) {
  ...
  getChunkedStage(runInfo) {
    ...
    if (alignment === 'FAILED' || hostFiltering === 'FAILED') {
      ...
      if (postProcess === 'LOADED') { return 'COMPLETE'; } else if (postProcess === 'FAILED') { return 'COMPLETE*'; }
  ...
  addFavIconClass(project) {
    return (
      <i data-status="favorite" data-fav={project.favorited} data-id={project.id} onClick={this.toggleFavorite} className={!project.favorited ? 'favorite fa fa-star-o' : 'favorite fa fa-star'} />
    );
  }
  ...
  compareSamples() {
    ...
    if (this.state.selectedSampleIndices.length) {
      const sampleParams = this.state.selectedSampleIndices;
      const sampleIds = this.state.selectedSampleIndices.map(idx => this.state.allSamples[idx].db_sample.id);
  ...
  applyExcluded(e, type, state_var) {
    ...
    const list = Object.assign([], this.state[state_var]);
    let index;
  ...
  generateTagList(state_all_options, state_selected_options, prefix, id_field = null, name_field = null, id_type = null) {
    return this.state[state_all_options].map((entry, i) => {
      const id = id_field ? entry[id_field] : entry;
      const name = name_field ? entry[name_field] : entry;
  ...
  renderTable(samples) {
    const project_id = this.state.selectedProjectId ? this.state.selectedProjectId : 'all';
    const search_field_width = 'col s3 no-padding';
    const search_field = (
      ...
      <div className="col s2 wrapper">
        <div className={this.state.displayDropdown ? 'metadata metadata-active' : 'metadata'} onClick={this.displayMetaDataDropdown}>
          <div className="metadata-dropdown">
            Filter
          </div><i className={this.state.displayDropdown ? 'fa fa-angle-up' : 'fa fa-angle-down'} />
        </div>
                    ...
                    this.state.hostGenomes.map((host, i) => (
                      <div key={i} className="options-wrapper">
                        <input
                          ...
                          type="checkbox"
                          data-id={host.id}
                          checked={this.state.selectedHostIndices.indexOf(host.id) < 0 ? '' : 'checked'}
                          value={this.state.selectedHostIndices.indexOf(i) != -1}
                   ...
                   this.state.tissueTypes.map((tissue, i) => (
                     <div key={i} className="options-wrapper">
                       <input
                         ...
                         className="filled-in"
                         data-status={tissue}
                         checked={this.state.selectedTissueFilters.indexOf(tissue) < 0 ? '' : 'checked'}
            ...
            {this.state.project ? (
              this.state.project.public_access ?
                ...
                <span>
                  ...
                  <a
                    href="#"
                    onClick={() => this.toggleProjectVisbility(this.state.project.id, 1)}
                  >
                      Make project public
                  </a>
                </span>
          ...
          <div className="members_list">
            ...
            <ul>
              { this.state.project_users.length > 0 ?
                  this.state.project_users.map(email => <li key={email}>{email}</li>)
                  : <li key="None">None</li>
              }
            </ul>
          </div>
    ...
    const filterStatus = (
      <div className="dropdown-status-filtering">
        ...
        <li className="filter-item" data-status="WAITING" onClick={this.handleStatusFilterSelect} ><a data-status="WAITING" className="filter-item waiting">Waiting</a><i data-status="WAITING" className="filter fa fa-check hidden" /></li>
        <li className="filter-item" data-status="UPLOADING" onClick={this.handleStatusFilterSelect}><a data-status="UPLOADING" className="filter-item uploading">In Progress</a><i data-status="UPLOADING" className="filter fa fa-check hidden" /></li>
        <li className="filter-item" data-status="CHECKED" onClick={this.handleStatusFilterSelect}><a data-status="CHECKED" className="filter-item complete">Complete</a><i data-status="CHECKED" className="filter fa fa-check hidden" /></li>
        <li className="filter-item" onClick={this.handleStatusFilterSelect} data-status="FAILED" ><a data-status="FAILED" className="filter-item failed">Failed</a><i data-status="FAILED" className="filter fa fa-check hidden" /></li>
        <li className="filter-item" data-status="ALL" onClick={this.handleStatusFilterSelect}><a data-status="ALL" className="filter-item all">All</a><i data-status="ALL" className="filter all fa fa-check hidden" /></li>
