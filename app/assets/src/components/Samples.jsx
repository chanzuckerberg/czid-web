...
class Samples extends React.Component {
  ...
  findSelectedColumns(selO) {
    ...
    for (let i = 0; i < selO.length; i++) {
      let column_name = selO.options[i].value;
  ...
  handleColumnSelectChange(e) {
    const selectedColumns = this.findSelectedColumns(e.target);
    this.setState({columnsShown: selectedColumns});
  }
  ...
  display_column_options(column_map, data_type) {
    const columnList = Object.keys(column_map)
    return columnList.map((option_name, i) => {
