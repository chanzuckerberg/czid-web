    ...
    return samples.map((sample, i) => {
      return (
        <tr key={i}>
          ...
          <td>{ !pipelineruns[i].pipeline_info ? 'N/A' : <a href={'/samples/' + sample.id}>{pipelineruns[i].pipeline_info.total_reads}</a>}</td>

          <td>{ !pipelineruns[i].pipeline_info ? 'N/A' : <a href={'/samples/' + sample.id}>{pipelineruns[i].pipeline_info.remaining_reads}</a>}</td>

          <td>{ !pipelineruns[i].pipeline_info ? 'N/A' : <a href={'/samples/' + sample.id}>{(pipelineruns[i].pipeline_info.remaining_reads/pipelineruns[i].pipeline_info.total_reads * 100).toFixed(2) }%</a>}</td>

          <td>{ !pipelineruns[i].pipeline_run ? 'N/A' : <a href={'/samples/' + sample.id}>{pipelineruns[i].pipeline_run.job_status}</a>}</td>
        </tr>
      )
    })
  }
