...
class ResultMonitorLoader
  ...
  def self.perform(pipeline_run_id, output)
    ...
    begin
      ...
      message = "SampleFailedEvent: Pipeline Run #{pr.id} for Sample #{pr.sample.id} by #{pr.sample.user.email} failed loading #{output} with #{pr.adjusted_remaining_reads} reads remaining after #{duration_hrs}. See: #{pr.status_url}"
