class ReportsController < ApplicationController
  ...
  # GET /reports/1.json
  def show
    @report_details = ReportsController.report_details(@report)
    @view_level = params[:view_level] ? params[:view_level] : 'genus'
    @taxonomy_details = ReportsController.taxonomy_details(@view_level, @report, params)
    @highest_tax_counts = ReportsController.highest_tax_counts(@view_level, @report)
  end
  ...
  class << self
    def external_report_info(report, view_level, params)
      data = {}
      data[:report_details] = ReportsController.report_details(report)
      data[:taxonomy_details] = ReportsController.taxonomy_details(view_level, report, params)
      data[:highest_tax_counts] = ReportsController.highest_tax_counts(view_level, report)
