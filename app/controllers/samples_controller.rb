class SamplesController < ApplicationController
  ...
  EDIT_ACTIONS = [:edit, :update, :destroy, :reupload_source, :resync_from_prod, :resync_from_staging, :kickoff_pipeline, :retry_pipeline,
  ...
  before_action :admin_required, only: [:reupload_source, :resync_from_prod, :resync_from_staging, :kickoff_pipeline, :retry_pipeline, :pipeline_runs]
  ...
  # PUT /samples/:id/resync_from_prod
  def resync_from_prod
    if Rails.env == 'staging'
      ...
      Resque.enqueue(InitiateS3EnvSync, @sample.id, 'prod')
    end
    respond_to do |format|
  ...
  # PUT /samples/:id/resync_from_staging
  def resync_from_staging
    if Rails.env == 'development'
      Resque.enqueue(InitiateS3EnvSync, @sample.id, 'staging')
    end
    respond_to do |format|
      format.html { redirect_to @sample, notice: "S3 data is being synced from staging." }
      format.json { head :no_content }
    end
  end
